...
Формат строки сообщения. Общий формат:

```text
    %[FORMAT]SUFFIX
```

`FORMAT` - общий формат, например: `%-02` для целого числа. Возможные значения SUFFIX:
...
День, месяц, год, час, минуты, секунды в численном виде.
